...
if [[ "$GRAFANA_ADMIN_PASSWORD" == "changeme" ]]; then
  echo "??  Grafana admin password is set to default (changeme). Set GRAFANA_ADMIN_PASSWORD to override."
fi
...
if [[ -n "$POSTGRES_EXPORTER_USER" && -n "$POSTGRES_EXPORTER_PASSWORD" ]]; then
  ...
  echo "??  Postgres exporter credentials missing; update payflow-secrets or set via Helm."
fi
...
if [[ -n "$RABBITMQ_EXPORTER_USER" && -n "$RABBITMQ_EXPORTER_PASSWORD" ]]; then
  ...
  echo "??  RabbitMQ exporter credentials missing; update payflow-secrets or set via Helm."
fi
...
if [[ -n "$RABBITMQ_EXPORTER_SERVICE" ]]; then
  ...
  echo "??  Could not find RabbitMQ exporter Service to annotate for Prometheus scraping."
fi
